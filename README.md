...
- **PoC — yt-dlp *as* the YouTube backend (full client loop):** on API 23, on-device yt-dlp does **search → resolve → play** in ExoPlayer, incl. **live HLS** (`SEARCH_OK`/`RESOLVE_OK`/`PLAYBACK_OK state=READY`) — see [`proof/44`](proof/44_apk_play_api23.md) (progressive) and [`proof/45`](proof/45_apk_search_play_api23.md) (search + live). Plus a live-chat→danmu bridge ([`prototype/`](prototype/)). Feasibility write-up: [`docs/yt-dlp-as-backend.md`](docs/yt-dlp-as-backend.md).
