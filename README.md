...
- **Full API-23 (fork):** [dewijones92/youtubedl-android](https://github.com/dewijones92/youtubedl-android) extends the fix to **ffmpeg + aria2c** too. ffmpeg validated *encoding* (`lavfi testsrc → mpeg4`) on a real API-23 device — see [`proof/42_apk_ffmpeg_api23.md`](proof/42_apk_ffmpeg_api23.md). Canonical recipe: the fork's `tools/patch_natives_api23.sh`.
- **App builds from our fork via JitPack (v0.18.6-api23-2):** `apk-repro` depends on `com.github.dewijones92.youtubedl-android:{library,ffmpeg}` straight from [`jitpack.io`](https://jitpack.io/#dewijones92/youtubedl-android) — reproducible on any machine, no binary overlay, no manifest `minSdk` override, no local publish. Verified end-to-end on API 23 (`SEARCH_OK`/`RESOLVE_OK`/`PLAYBACK_OK`). Depending on the real fork surfaced + fixed two fork bugs: (1) yt-dlp-driven **ffmpeg/ffprobe wouldn't link** on API 23 (`in6addr_any`/`strchrnul`, API-24+ libc symbols needed by ffmpeg's `libzmq`/`libidn2`, resolvable only via an `LD_PRELOAD`ed shim) — sponsor cut now runs (1511 s → 1458.29 s); (2) **quickjs JS runtime undetected** (bionic linker warnings polluted yt-dlp's version probe) — fixed with a stderr-suppressing qjs wrapper, yt-dlp now sees `quickjs-2025-04-26`. See [`proof/48`](proof/48_app_depends_on_fork_api23.md).
- **Unified client (both goals in one app):** `apk-repro` searches YouTube and lets you **tap = play** or **long-press = download without sponsor** — on Android 6.0. See [`proof/47`](proof/47_apk_unified_client_api23.md).
