...
- **App depends on the fork (v0.18.5):** `apk-repro` now consumes the fork's AAR directly (via `mavenLocal`) — no binary overlay, no manifest `minSdk` override. That surfaced a real fork bug: yt-dlp-driven **ffmpeg/ffprobe wouldn't link** on API 23 (`in6addr_any`/`strchrnul` — API-24+ libc symbols — needed by ffmpeg's `libzmq`/`libidn2`, only resolvable via an `LD_PRELOAD`ed shim). **Fixed** (shim symbols + `LD_PRELOAD` in `YoutubeDL.execute()`): sponsor cut runs (1511 s → 1458.29 s). See [`proof/48`](proof/48_app_depends_on_fork_api23.md).
