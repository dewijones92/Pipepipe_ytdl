...
- **PoC — yt-dlp *as* the YouTube backend:** on API 23, on-device yt-dlp resolves a stream URL and **ExoPlayer plays it** (`RESOLVE_OK` + `PLAYBACK_OK state=READY`) — see [`proof/44_apk_play_api23.md`](proof/44_apk_play_api23.md). Plus a live-chat→danmu bridge ([`prototype/`](prototype/)). Feasibility notes for replacing NewPipeExtractor/MediaServiceCore with yt-dlp in [`docs/yt-dlp-as-backend.md`](docs/yt-dlp-as-backend.md).
